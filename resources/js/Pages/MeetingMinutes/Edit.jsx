...
export default function Edit({ meeting, employees }) {
    // Map agenda items dari server — bentuk deadline menjadi yyyy-mm-dd untuk input date
    ...
    const { data, setData, put, processing, errors } = useForm({
        ...
        agenda_items: mapAgendaItems(meeting.agenda_items),
        _method: 'put',
    });
    ...
    const addAgendaItem = () => {
        setData('agenda_items', [
            ...data.agenda_items,
            { id: null, discussion: '', decision: '', pic: [], deadline: '', status: 'pending' },
        ]);
    };
    ...
    const submit = (e) => {
        e.preventDefault();
        put(route('meeting-minutes.update', meeting.id));
    };
    ...
    return (
        <AdminLayout title="Edit Notulen Rapat">
            <Head title="Edit Notulen Rapat" />

                    <div className="mb-6 flex items-center justify-between">
                        <h1 className="text-2xl font-bold text-gray-900">Edit Notulen Rapat</h1>
                        <Link
                    ...
                    <form onSubmit={submit} className="space-y-6">
                        ...
                        <div className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
                            ...
                            <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                                <div>
                                    <label className={labelClass} htmlFor="title">Judul Notulen *</label>
                                    <input
                                        id="title"
                                        type="text"
                                        value={data.title}
                                ...
                                <div>
                                    <label className={labelClass} htmlFor="division">Divisi *</label>
                                    <input
                                        id="division"
                                        type="text"
                                        value={data.division}
                                ...
                                <div>
                                    <label className={labelClass} htmlFor="meeting_date">Tanggal Rapat *</label>
                                    <input
                                        id="meeting_date"
                                        type="date"
                        ...
                        {/* Attendees */}
                        <div className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
                            <h2 className="mb-4 text-lg font-semibold text-gray-900">Peserta Rapat *</h2>
                            <MultiSelectEmployee
                                employees={employees || []}
                        ...
                        <div className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
                            ...
                            <div className="space-y-4">
                                {data.agenda_items.map((item, index) => (
                                    <div key={item.id ?? `new-${index}`} className="rounded-lg border border-gray-200 bg-gray-50 p-4">
                                        ...
                                        <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                                            <div className="md:col-span-2">
                                                <label className={labelClass}>Pembahasan *</label>
                                                <textarea
                                            ...
                                            <div>
                                                ...
                                                <select
                                                    value={item.status}
                                                    onChange={(e) => updateAgendaItem(index, 'status', e.target.value)}
                                                    className={inputClass}
                                                >
