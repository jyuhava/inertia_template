            ...
            <div className="space-y-6">
                <Box variant="black" className="relative overflow-hidden">
                    ...
                    <div className="relative flex flex-col justify-between gap-4 md:flex-row md:items-start">
                        <div>
                            <p className="text-xs font-bold uppercase tracking-widest text-neutral-400">Kartu Hasil Studi</p>
                            <h1 className="mt-2 text-2xl font-bold md:text-3xl">
                                {periodeKrs.semester?.nama_semester} {periodeKrs.tahun_ajaran?.tahun_mulai}/{periodeKrs.tahun_ajaran?.tahun_selesai}
                            </h1>
                            <p className="mt-1 text-sm text-neutral-300">{periodeKrs.nama_periode}</p>
                        </div>
                            ...
                            <table className="min-w-full text-sm">
                                ...
                                <tbody className="divide-y divide-neutral-100">
                                    {krs.map((item, index) => (
                                        <tr key={index} className="hover:bg-neutral-50">
                                            <td className="py-4 pr-4 text-neutral-900">{index + 1}</td>
                                            <td className="py-4 pr-4 font-bold text-neutral-900">{item.kode_mata_kuliah}</td>
                                            <td className="py-4 pr-4 text-neutral-900">{item.nama_mata_kuliah}</td>
                                            <td className="py-4 pr-4 text-center font-bold text-neutral-900">{item.sks}</td>
                                            <td className="py-4 pr-4 text-center text-neutral-700">{item.nilai_angka || '-'}</td>
                                            <td className="py-4 pr-4 text-center">
                                                <GradeBadge nilai={item.nilai_huruf} />
                                            </td>
                                            <td className="py-4 pr-4 text-center text-neutral-700">{item.bobot.toFixed(2)}</td>
                                            <td className="py-4 text-center font-bold text-neutral-900">{item.mutu.toFixed(2)}</td>
                                        </tr>
                                    ))}
                                </tbody>
                                <tfoot className="border-t border-neutral-200 bg-neutral-50">
                                    <tr>
                                        ...
                                        <td className="py-4 pr-4 text-center text-sm font-bold text-neutral-900">{totalSks}</td>
                                        <td className="py-4 pr-4 text-center text-sm text-neutral-500">-</td>
                                        <td className="py-4 pr-4 text-center text-sm text-neutral-500">-</td>
                                        <td className="py-4 pr-4 text-center text-sm text-neutral-500">-</td>
                                        <td className="py-4 text-center text-sm font-bold text-neutral-900">{totalMutu.toFixed(2)}</td>
                                    </tr>
                                </tfoot>
                            </table>
                        </div>
