...
function SemesterBadge({ nama_semester }) {
    const map = {
        Ganjil: 'bg-neutral-100 text-neutral-700 border-[#e4e4e7]',
        Genap: 'bg-black text-white border-black'
    };
    return (
        <span className={`inline-flex px-3 py-1 text-[10px] font-semibold uppercase tracking-wider border ${map[nama_semester] || 'bg-neutral-100 text-neutral-700 border-[#e4e4e7]'}`}>
            {nama_semester}
        </span>
    );
}
...
export default function Index({ semesters, filters }) {
    ...
    const handleSearch = (e) => {
        e.preventDefault();
        router.get(route('admin.semester.index'), { search }, {
            preserveState: true,
            replace: true,
        });
    };
    ...
    const confirmDelete = () => {
        if (semesterToDelete) {
            router.delete(route('admin.semester.destroy', semesterToDelete.id));
            ...
            <div className="space-y-6">
                ...
                <Box variant="dark" className="relative overflow-hidden">
                    <div className="relative z-10">
                        ...
                        <h1 className="text-2xl font-bold tracking-tight text-white">SEMESTER</h1>
                        <p className="text-sm text-neutral-400 mt-1">Kelola data semester akademik</p>
                    </div>
                </Box>

                {/* Controls */}
                <Box>
                    <SectionTitle action={<ActionButton href={route('admin.semester.create')} variant="primary">+ Tambah Semester</ActionButton>}>
                        Daftar Semester
                    </SectionTitle>
                        ...
                        <div className="flex gap-2">
                            ...
                            {search && (
                                <ActionButton
                                    ...
                                    onClick={() => {
                                        setSearch('');
                                        router.get(route('admin.semester.index'));
                                    }}
                                    variant="secondary"
                                >
                                    Reset
                                        ...
                                        <tr key={semester.id} className="hover:bg-[#fafafa] transition-colors">
                                            ...
                                            <td data-label="Aksi" className="px-5 py-4 whitespace-nowrap text-right">
                                                <div className="flex justify-end gap-2">
                                                    ...
                                                    <ActionButton onClick={() => handleDelete(semester)} variant="danger" size="sm">Hapus</ActionButton>
                                                </div>
                                            </td>
                                        </tr>
                                    ))
                    ...
                    {semesters.links && semesters.links.length > 3 && (
                        <div className="px-5 py-4 border-t border-[#e4e4e7] flex flex-col sm:flex-row items-center justify-between gap-4">
                            <div className="text-xs text-neutral-500 uppercase tracking-wider">
                                Menampilkan {semesters.from || 0} sampai {semesters.to || 0} dari {semesters.total} data
                            </div>
            ...
            <Modal show={showDeleteModal} onClose={() => setShowDeleteModal(false)}>
                <div className="p-6 border border-[#e4e4e7] bg-white">
                    ...
                    <p className="text-sm text-neutral-600 mb-6">
                        Apakah Anda yakin ingin menghapus semester "{semesterToDelete?.nama_semester}" tahun ajaran "{semesterToDelete?.tahun_ajaran?.nama_tahun_ajaran}"? 
