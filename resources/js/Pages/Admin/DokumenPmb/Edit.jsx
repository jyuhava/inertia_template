...
export default function Edit({ dokumenPmb }) {
    const { data, setData, put, processing, errors } = useForm({
        nama_dokumen: dokumenPmb.nama_dokumen || '',
        kode_dokumen: dokumenPmb.kode_dokumen || '',
        deskripsi: dokumenPmb.deskripsi || '',
        jenis_file: dokumenPmb.jenis_file || 'pdf',
        max_size_kb: dokumenPmb.max_size_kb || '',
        wajib: dokumenPmb.wajib || false,
        urutan: dokumenPmb.urutan || '',
    });

    const submit = (e) => {
        e.preventDefault();
        put(route('admin.dokumen-pmb.update', dokumenPmb.id));
    };

    const fileTypes = ['pdf', 'jpg', 'jpeg', 'png', 'doc', 'docx'];

    return (
        <AdminLayout title="Edit Dokumen PMB">
            <Head title="Edit Dokumen PMB" />

            <div className="space-y-6">
                <Box variant="black" className="flex flex-col md:flex-row md:items-end md:justify-between gap-4">
                    <div>
                        <p className="text-[10px] font-bold uppercase tracking-widest text-white/75 mb-2">Dokumen PMB</p>
                        <h1 className="text-2xl font-bold text-white">Edit Dokumen PMB</h1>
                        <p className="text-sm text-white/75 mt-1">{dokumenPmb.nama_dokumen} ({dokumenPmb.kode_dokumen})</p>
                    </div>
