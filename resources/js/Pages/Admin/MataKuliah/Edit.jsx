...
export default function Edit({ mataKuliah, prodis, kategoris, mataKuliahOptions }) {
    const { data, setData, put, processing, errors } = useForm({
        kode_mata_kuliah: mataKuliah.kode_mata_kuliah || '',
        nama_mata_kuliah: mataKuliah.nama_mata_kuliah || '',
        short_name: mataKuliah.short_name || '',
        english_name: mataKuliah.english_name || '',
        sks: mataKuliah.sks || '',
        theory_credits: mataKuliah.theory_credits || '',
        practical_credits: mataKuliah.practical_credits || '',
        field_credits: mataKuliah.field_credits || '',
        semester: mataKuliah.semester || '',
        prodi_id: mataKuliah.prodi_id || '',
        jenis: mataKuliah.jenis || 'Wajib',
        course_type: mataKuliah.course_type || '',
        kategori_mata_kuliah_id: mataKuliah.kategori_mata_kuliah_id || '',
        deskripsi: mataKuliah.deskripsi || '',
        status: mataKuliah.status || 'aktif',
        prasyarat_ids: (mataKuliah.prasyarats || []).map(p => p.id),
    });

    const submit = e => { e.preventDefault(); put(route('admin.mata-kuliah.update', mataKuliah.id)); };
        ...
        <AdminLayout title="Edit Mata Kuliah">
            ...
            <div className="p-6 space-y-6 bg-neutral-50 min-h-dvh">
                <div className="bg-gradient-to-r from-slate-800 to-indigo-700 p-6 text-white flex justify-between">
                    <div>
                        <p className="text-xs uppercase tracking-widest">Mata Kuliah</p>
                        <h1 className="text-2xl font-bold">{mataKuliah.kode_mata_kuliah} - {mataKuliah.nama_mata_kuliah}</h1>
                    </div>
                    <Link href={route('admin.mata-kuliah.index')} className="text-sm">Kembali</Link>
                </div>
                <form onSubmit={submit} className="space-y-6">
                    <Section title="Identitas Mata Kuliah">
                        <Input label="Kode Mata Kuliah" field="kode_mata_kuliah" data={data} setData={setData} errors={errors} required />
                    ...
                    <Section title="Jenis dan Kategori">
                        ...
                        <Input label="Tipe Mata Kuliah" field="course_type" data={data} setData={setData} errors={errors} />
