...
export default function Create() {
    const { data, setData, post, processing, errors } = useForm({ kode: '', nama: '', gedung: '', lantai: '', kapasitas: '', tipe: 'kelas', status: 'aktif' });
    const submit = e => { e.preventDefault(); post(route('admin.ruangan.store')); };

    return (
        <AdminLayout title="Tambah Ruangan">
            ...
            <div className="p-6 space-y-6 bg-neutral-50 min-h-dvh">
                ...
                <form onSubmit={submit} className="bg-white border p-6 grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <p className="text-xs text-red-600">{errors.kapasitas}</p>
                    </div>
                    <div>
                    ...
                    <div className="md:col-span-2">
                        <button disabled={processing} className="bg-black text-white px-5 py-3 text-sm font-bold">{processing ? 'Menyimpan...' : 'Simpan Ruangan'}</button>
                    </div>
                </form>
            </div>
        </AdminLayout>
