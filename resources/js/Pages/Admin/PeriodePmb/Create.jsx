...
export default function Create() {
    const { data, setData, post, processing, errors } = useForm({
        ...
        kuota_total: '100',
        persyaratan: "1. Lulusan SMA/MA/SMK/Sederajat\n2. Nilai rata-rata rapor minimal 7.0\n3. Sehat jasmani dan rohani\n4. Tidak buta warna (untuk prodi tertentu)",
        keterangan: '',
    });

    const submit = (e) => {
        e.preventDefault();
        post(route('admin.periode-pmb.store'));
    };

    return (
        <AdminLayout title="Tambah Periode PMB">
            <Head title="Tambah Periode PMB" />

            <div className="space-y-6">
                <Box variant="black" className="flex flex-col md:flex-row md:items-end md:justify-between gap-4">
                    <div>
                        <p className="text-[10px] font-bold uppercase tracking-widest text-neutral-400 mb-2">Periode PMB</p>
                        <h1 className="text-2xl font-bold text-white">Tambah Periode PMB Baru</h1>
                        <p className="text-sm text-neutral-400 mt-1">Atur jadwal dan kuota penerimaan mahasiswa baru.</p>
                    </div>
